...
<head>
    ...
    <title>Laporan Rincian Armada Perusahaan {{ $company->name }}</title>
    <style>
        @page {
            margin: 15px;
        }
...
<body>
    ...
    <div class="container">
        <div class="header-title">
            <h1>LAPORAN RINCIAN ARMADA PERUSAHAAN {{ strtoupper($company->name) }}</h1>
            <p>PT Putra Harta Logistik - Fleet Management System</p>
        </div>

        <div class="info">
            <p><strong>Perusahaan Armada:</strong> {{ $company->name }} (Kode: {{ $company->code }} | Tipe: {{ $company->type ?? '-' }})</p>
            <p><strong>Rekening Bank:</strong> {{ $company->bankName ?? '-' }} - {{ $company->accountNumber ?? '-' }} (PPh: {{ $company->pph ? number_format($company->pph, 2, ',', '.') . '%' : '0%' }})</p>
            <p><strong>Filter Merek Armada:</strong> {{ $fleetBrandName ?? 'Semua Merek' }}</p>
            <p><strong>Filter Tipe Armada:</strong> {{ $fleetTypeName ?? 'Semua Tipe' }}</p>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width: 14%;">No. Rangka</th>
                </tr>
            </thead>
            <tbody>
