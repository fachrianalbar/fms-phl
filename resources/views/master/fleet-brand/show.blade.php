            ...
            <div class="card-header bg-white py-3 border-bottom d-flex justify-content-between align-items-center"
                ...
                <div>
                    <h4 class="mb-1 fw-bold text-dark d-flex align-items-center gap-2">
                        ...
                        Detail Merek Armada: {{ $brand->name }}
                    </h4>
                    <small class="text-muted">Profil merek armada dan daftar armada terdaftar dengan merek ini</small>
                </div>

                <div class="d-flex align-items-center gap-2">
                    {{-- Tombol Export Excel --}}
                    <a href="{{ route($view . 'detail-export-excel', $brand->id) }}" target="_blank" id="export-excel"
                    ...
                    {{-- Tombol Export PDF --}}
                    <a href="{{ route($view . 'detail-export-pdf', $brand->id) }}" target="_blank" id="export-pdf"
                        ...
                        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
                            <div class="d-flex align-items-center gap-3">
                                <div class="avatar-badge-entity">
                                    <i class="mdi mdi-truck-flatbed fs-22 text-white"></i>
                                </div>
                                <div>
                                    <div class="d-flex align-items-center gap-2">
                                        <span class="fs-16 fw-bold text-dark">{{ $brand->name }}</span>
                                    ...
                                    <div class="text-muted small mt-1">
                                        <i class="mdi mdi-identifier me-1"></i>Kode Merek: <span class="font-monospace fw-semibold">{{ $brand->code }}</span>
                                        <span class="mx-2">•</span>
                                        <i class="mdi mdi-calendar-check me-1"></i>Terdaftar: <span class="font-monospace">{{ $brand->created_at ? $brand->created_at->format('d/m/Y H:i') : '-' }}</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <table class="table align-middle w-100 mb-0" id="dt-fleets">
                        <thead>
                            <tr>
                                ...
                                <th style="width: 15%;">Tipe Armada</th>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function syncExportUrls() {
                ...
                $('#export-excel').attr('href', "{{ route($view . 'detail-export-excel', $brand->id) }}" + qs);
                $('#export-pdf').attr('href', "{{ route($view . 'detail-export-pdf', $brand->id) }}" + qs);

                // Update periode badge di banner
                if (startDate || endDate) {
                    $('#filterPeriodBadge').removeClass('d-none').addClass('d-flex');
                    const startText = startDate ? startDate : 'Awal';
                    const endText = endDate ? endDate : 'Sekarang';
                    $('#filterPeriodText').text(startText + ' s/d ' + endText);
                } else {
                    $('#filterPeriodBadge').addClass('d-none').removeClass('d-flex');
                }
            }
            ...
            const table = $('#dt-fleets').DataTable({
                ...
                pageLength: 25,
                ajax: {
                    url: "{{ route('dt.fleet-brand.fleets', $brand->id) }}",
                    data: function(d) {
                ...
                columns: [
                    ...
                    { data: 'year', className: 'text-center align-middle' },
