...
use App\Exports\FleetBrandDetailExport;
use App\Exports\FleetBrandExport;
...
class FleetBrandController extends Controller
{
    ...
    public function __construct(FleetBrandService $fleetBrandSvc, MenuService $menuSvc)
    {
        $this->service = $fleetBrandSvc;
        $this->title = 'Fleet Brand';
        $menu = $menuSvc->getByName('Fleet Brand');
        if ($menu) {
            $this->title = (Auth::check() && Auth::user()->languange == 'en') ? $menu->name : $menu->nama;
        }
        $this->view = 'master.fleet-brand.';
    }
    ...
    public function show(string $id)
    {
        ...
        $totalFleets = Fleet::where('fleetBrandCode', $brand->code)->count();
        $totalTypes = Fleet::where('fleetBrandCode', $brand->code)
            ->whereNotNull('fleetTypeCode')
            ->distinct('fleetTypeCode')
            ->count('fleetTypeCode');
        $totalCompanies = Fleet::where('fleetBrandCode', $brand->code)
    ...
    /**
     * Datatable for Fleet Brand index.
     */
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    return '<a href="'.$showUrl.'" class="btn btn-icon btn-sm bg-info-subtle me-1" data-bs-toggle="tooltip" title="Detail">'
                        ...
                        .'<button type="button" class="btn btn-icon btn-sm bg-primary-subtle me-1 js-edit-fleet-brand"'
                        .' data-bs-toggle="modal" data-bs-target="#fleetBrandCrudModal"'
                        .' data-name="'.$brandName.'" data-action="'.$escapedUpdateUrl.'"'
                        .' title="Edit">'
    ...
    /**
     * Datatable for Fleets belonging to a specific Fleet Brand.
     */
    public function datatableFleets(string $id, Request $request)
    {
        if ($request->ajax()) {
            ...
            $query = Fleet::query()
                ->with(['type', 'company'])
                ->where('fleetBrandCode', $brand->code);
            ...
            return Datatables::of($query)
                ...
                ->rawColumns(['plateNumber', 'code', 'typeName', 'companyName', 'year', 'engineNumber', 'frameNumber'])
                ->toJson();
        }
    }

    /**
     * Bangun query brand dengan filter nama dan tanggal.
     */
    private function buildFilteredQuery(Request $request)
    {
        $query = $this->service->findAllQuery();

        if ($request->filled('brandCode')) {
            $query->where('code', $request->brandCode);
        }
    ...
    /**
     * Export Excel for Fleet Brand list.
     */
    public function exportExcel(Request $request)
    {
        return Excel::download(new FleetBrandExport($request), 'Fleet-Brand-Report.xlsx');
    }

    /**
     * Export PDF for Fleet Brand list.
     */
    public function exportPdf(Request $request)
    {
        ...
        if ($request->filled('brandCode')) {
            $selectedBrand = FleetBrand::where('code', $request->brandCode)->first();
            $brandFilterName = $selectedBrand ? $selectedBrand->name : null;
        }
        ...
        $mpdf->WriteHTML(
            view($this->view.'report.fleet-brand-pdf')
                ->with('rows', $rows)
        ...
        return $mpdf->Output('Fleet-Brand-Report.pdf', 'I');
    }

    /**
     * Export Excel for Fleets under specific Fleet Brand.
     */
    public function exportDetailExcel(string $id, Request $request)
    {
        ...
        return Excel::download(new FleetBrandDetailExport($brand->code, $request), 'Fleet-Brand-'.$brand->code.'-Fleets.xlsx');
    }

    /**
     * Export PDF for Fleets under specific Fleet Brand.
     */
    public function exportDetailPdf(string $id, Request $request)
    {
        ...
        $fleetTypeName = null;
        if ($request->filled('fleetTypeCode')) {
            $fleetTypeName = optional(FleetType::where('code', $request->fleetTypeCode)->first())->name;
        }
        ...
        $mpdf->WriteHTML(
            view($this->view.'report.fleet-brand-detail-pdf')
                ->with('brand', $brand)
                ->with('rows', $rows)
        ...
        return $mpdf->Output('Fleet-Brand-'.$brand->code.'-Fleets.pdf', 'I');
    }
}
