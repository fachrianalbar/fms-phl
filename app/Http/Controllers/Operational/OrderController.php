...
class OrderController extends Controller
{
    ...
    public function storeOrderDriver(Request $request)
    {
        ...
        try {
            ...
            // Fetch order and check permission/status
            $order = \App\Models\Operational\Order::where('code', $request->orderCode)->first();
            ...
            if (! in_array(auth()->user()->roleCode, ['SPRADMIN', 'SPRUSER']) && $order->status >= 4) {
                DB::rollBack();
                $message = 'Tidak diizinkan mengganti supir untuk status order ini';
            ...
            // Update active driver on order
            $order->driverCode = $request->driverCode;
            $order->save();
    ...
    public function calculateOrderPrice(Request $request)
    {
        try {
            ...
            if ($routeCode) {
                ...
                if ($route) {
                    // Price calculations (always show these)
