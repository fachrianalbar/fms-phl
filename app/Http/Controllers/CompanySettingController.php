...
class CompanySettingController extends Controller
{
    ...
    {
        $this->service = $companySettingService;
        $this->title = "Company Setting";
        $this->view = "administrator.company-setting.";
    }
    ...
    public function create()
    {
        ...
        $companyData = collect($data[0])
            ->pluck('company_name')->unique()
            ->values()
            ->all();

        $companySetting = CompanySetting::groupBy('id', 'name')->pluck('name')->toArray();

        $company = array_diff($companyData, $companySetting);


        // $data = array_map(function ($item, $index) use ($company) {
        //     $item['companyName'] = $company[$index % count($company)];
        //     return $item;
        // }, $dataApi[0], array_keys($dataApi[0]));
