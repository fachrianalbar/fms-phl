(function ($) {
    ...
    if ($("#pageWrapper").hasClass("compact-wrapper")) {
        ...
        var currentPath = window.location.pathname; // Ambil URL saat ini

        const slashCount = (currentPath.match(/\//g) || []).length;

        // Jika jumlah '/' lebih dari 2, lakukan pemrosesan
        if (slashCount > 2) {
            currentPath = currentPath.split("/").slice(0, 3).join("/");
        }

        $(".sidebar-wrapper nav ul li a").each(function () {
            var link = new URL($(this).attr("href"), window.location.origin)
                .pathname;

            if (link && currentPath === link) {
                $(this).addClass("active"); // Tambahkan class active pada menu aktif

                // Pastikan parent menu juga mendapat class 'active' agar tetap terbuka
